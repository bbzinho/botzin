...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'FN:ZEUS\n' //NAMA LU
            + 'ORG:Owner Bot;\n' // NAMA CONTACT OWNER
            + 'TEL;type=CELL;type=VOICE;waid=5571982709851:+55 71 98370-9851\n' // NOMER OWNER
            + 'END:VCARD'
prefix = '#'//PREFIX BOT
blocked = ['5571982709851']//NOMER BLOCK
limitawal = '50'
cr = '*BOT WHATSAPP*'
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '[❗] só grupos kraio!',
					ownerG: '[❗] só o dono do gp pd!',
					ownerB: '[❗] só o lalpha usa cria!',
					Badmin: '[❗] coloca o meu adm😔🚮!',
			...
			const ownerNumber = ["557198270851@s.whatsapp.net"] // Nomer Owner
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5571982709851@s.whatsapp.net'//Nomer Owner
                        const isEventon = isGroup ? event.includes(from) : false
                        const isRegister = checkRegisteredUser(sender)
                        pushname = client.contacts[sender] != undefined ? client.contacts[sender].vname || client.contacts[sender].notify : undefined

                        // Options
                        const botName = 'ALPHA'//Nama Bot
                        const ownerName = 'ALPHA<3'//Nama Owner
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `rebaixado do cargo de administrador:\n`
							teks += `@_.split('@')[0]`
						}
                  ...
                  case 'promover':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'foi promovido como administrador :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
				if (args.length < 1) return reply('e o link ??')
		...
		case 'glass':                                        
               ...
               if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌n consegui converter o vídeo pra mp3❌')
               ...
               if (data.error) return reply(data.error)
				...
				case 'audio':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual é a sigla do pais?', text, {quoted: mek})
					...
					if (args.length < 2) return client.sendMessage(from, 'kd a porra do texto ?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'tm':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*[ALPHA BROADCAST ]*\n\n${body.slice(4)}`)
						}
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'nem vai fazer falta otário', text) // ur cods
					}, 0)
                     break

				case 'chord':
					if (args.length < 1) return reply('q msc ?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					chorr = body.slice(7)					
           ...
           break
				...
				case 'remover':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'esse tomo no cu ksksk', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				case 'clone':
					...
					break
			                ...
			                if (!isBotGroupAdmins) return reply(ind.badmin())
                                        ...
                                        client.sendMessage(from, 'Sucesso!', text, {quoted: mek})
				        break
                                case 'setdesc':
                                        if (!isGroup) return reply(ind.groupo())
			                if (!isGroupAdmins) return reply(ind.admin())
